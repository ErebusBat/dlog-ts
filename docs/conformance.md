...
| TAIL-24 | Run `tail --width 20` on a section with long lines.                                                         | Displayed date, heading, and every line are truncated to 20 columns, with `…` appended, including long section lines and short lines that exceed the selected width due to leading styles. |
| TAIL-25 | Run `tail --color=always --width 15` on long lines.                                                         | Width is measured after removing ANSI escapes, and truncated styled lines still terminate with `ESC[0m` before `…`, so terminal style is not leaked into subsequent output.                                      |
| TAIL-26 | Run `tail --width 20 --no-truncate` and `tail --no-truncate --width 20`.                                    | Last truncation flag in argument order wins (`--no-truncate` disables truncation, while earlier `--width` does not force truncation by itself).                                                             |
| TAIL-27 | Config includes `[tail] truncate = true` and optionally `width = 20`.                                         | Truncation defaults follow config; explicit CLI `--no-truncate` or `--width` override config. In terminal mode with no explicit width, `[tail] width` applies.                                                 |
| TAIL-28 | Run `tail --truncate` with no measurable terminal width and no `--width`.                                    | Fail nonzero with a clear error that a width is required, while `tail --truncate --width 20` (or `COLUMNS` set) succeeds.                                                                                     |
...
| TAIL-32 | Section contains ``Ran `dlog tail`; keep `unfinished``. | The matched inline code renders without backticks using inverse terminal defaults; the unterminated backtick remains literal. |
...
| -------- | ---------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| THEME-01 | No `DLOG_THEME`, config theme, or sibling `theme.toml`; run `dlog theme --dump --silent`.                              | Status `0`; stdout is complete valid `dlog-theme/v1` TOML for the built-in theme, with eleven role tables, every property explicit, and every `inherit = false`; stderr empty. |
